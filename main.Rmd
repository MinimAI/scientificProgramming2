...
```
## Importing the data.
First, I import the activity scores. To properly import the activity scores matrix, 2 import function calls are needed: one to import the header and one to import the data.
```{r}
# import the header and the activity scores
headers <- read.csv('data/aid624202.csv', header=FALSE, nrows=1, as.is=TRUE, row=1)
data.raw <- read.csv('data/aid624202.csv',skip=6,header=FALSE,row=1)
...
selectedInactives <- sample(inactives, 200)
selectedActives   <- sample(actives, 12000)
...
iter <- iload.molecules('/Users/felix/Google Drive/Uni/Systems Biology/Year 2/Scientific Programming/scientificProgramming2/data/aid624202.sdf', type='sdf')
# creates a txtProgressBar object to display the progress of importing the selected molecules
pb <- txtProgressBar(min = 0, max = 12200, style = 3)
...
write.csv(cbind(molSIDs, descs), file = "data/descriptors12200.csv")
```
...
x <- read.csv('data/descriptors12200.csv',row=2)
...
```
Finally, I plot the regression plots of the train set and the test set.
```{r}
